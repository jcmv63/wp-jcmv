...
final class PostTypes {
	...
	/**
	 * Clés de disponibilité déclarative d'un produit (ADR-005 : pas de
	 * décompte de stock). Le premier élément fait office de valeur par défaut.
	 *
	 * Les clés seules ici, les libellés dans disponibilites() : une constante
	 * de classe ne peut pas appeler __(), et des libellés en dur ici
	 * échapperaient définitivement au fichier de traduction.
	 *
	...
	/**
	 * Libellés traduisibles des disponibilités, dans l'ordre d'affichage.
	 *
	 * @return array<string,string> clé => libellé.
	 */
	public static function disponibilites(): array {
		$labels = array(
			'disponible'   => __( 'Disponible', 'wp-jcmv' ),
			'sur-commande' => __( 'Sur commande', 'wp-jcmv' ),
			'epuise'       => __( 'Épuisé', 'wp-jcmv' ),
		);

		// Périmètre et ordre pilotés par la constante : une clé ajoutée sans
		// son libellé s'affiche sous sa clé plutôt que de disparaître en
		// silence de la liste déroulante.
		$out = array();
		foreach ( self::DISPONIBILITES as $key ) {
	...
	/** Libellé d'une disponibilité, avec repli sur la valeur par défaut. */
	public static function dispo_label( string $key ): string {
		$labels = self::disponibilites();

		return $labels[ $key ] ?? $labels[ self::DISPONIBILITES[0] ];
	}
	...
	public static function sanitize_dispo( $value ): string {
		...
		// Validation sur la constante, sans passer par disponibilites() : cette
		// fonction est appelée pour chaque produit du catalogue, inutile d'y
		// déclencher des recherches de traduction.
		return in_array( $value, self::DISPONIBILITES, true ) ? $value : self::DISPONIBILITES[0];
	}
